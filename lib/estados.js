...
function leerPrestadero(t) {
  ...
  return {
    ...
    disponible:     buscar(t, 'Dinero Disponible', { porDefecto: null }),
...
function leerBriq(t) {
  ...
  return {
    ...
    // El efectivo sin invertir es el "Saldo briq.mx" al cierre, que es el
    // "Al final del periodo" que viene justo después de la línea de salidas.
    disponible:   (function () {
      const m = t.match(/-\s*salidas\s*:?\s*\$?[\d,.]+\s*\n?\s*Al final del periodo\s*:?\s*\$?([\d,.]+)/i);
...
function leerYTP(t) {
  ...
  return {
    ...
    disponible:    buscar(t, 'final del periodo:', { porDefecto: null }),
    principal:     0,
...
const CONCEPTOS = [
  ...
  ['Dinero disponible',    'Cuenta',  r => r.disponible]
];
